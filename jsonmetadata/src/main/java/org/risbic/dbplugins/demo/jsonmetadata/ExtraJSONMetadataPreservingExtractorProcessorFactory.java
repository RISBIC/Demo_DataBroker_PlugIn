...
public class ExtraJSONMetadataPreservingExtractorProcessorFactory implements DataFlowNodeFactory
{
    public ExtraJSONMetadataPreservingExtractorProcessorFactory(String name, Map<String, String> properties)
    {
        _name       = name;
        ...
        throws InvalidClassException, InvalidMetaPropertyException, MissingMetaPropertyException
    {
        if (dataFlowNodeClass.equals(DataProcessor.class))
        {
            if (metaProperties.isEmpty())
            {
                ...
                propertyNames.add(ExtraJSONMetadataPreservingExtractorProcessor.METADATABLOB_ID_PROPNAME);
                propertyNames.add(ExtraJSONMetadataPreservingExtractorProcessor.LOCATION_PROPNAME);
        ...
        throws InvalidClassException, InvalidNameException, InvalidMetaPropertyException, MissingMetaPropertyException, InvalidPropertyException, MissingPropertyException
    {
        ...
        {
            if (metaProperties.isEmpty())
                return (T) new ExtraJSONMetadataPreservingExtractorProcessor(name, properties);
            else
                throw new InvalidMetaPropertyException("No metaproperties expected", null, null);
        }
        else
